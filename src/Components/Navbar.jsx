...
const Navbar = () => {
  ...
  let ref = useRef();
  const [auth, name, setAuth] = useAuthentication();
  const paramObj = {
    params: {
      q: query,
    },
      ...
      <Box bg={useColorModeValue("#404040;", "#404040;")} px={7}>
        <Flex h={16} alignItems={"center"} justifyContent={"space-between"}>
          <IconButton
            ...
            icon={
              isOpen ? (
                <CloseIcon color={"#D7B14A"} />
              ) : (
                <HamburgerIcon color={"#D7B14A"} />
              )
            }
          ...
          <HStack spacing={8} alignItems={"center"}>
            <Box>
              <Link to="/">
                <Image
                  src="https://i.postimg.cc/YSKBpWR6/gz.png"
                  width={200}
                  alt=""
                />
              </Link>
              ...
              {auth ? (
                <Menu>
                  ...
                  <MenuList>
                    ...
                    >
                      <Flex alignItems={"center"} gap="20px" fontSize={"18px"}>
                        Logout <SlLogout />
                      </Flex>
                    </MenuItem>
                    <MenuItem>
                      <Flex alignItems={"center"} gap="20px" fontSize={"18px"}>
                        <Link to="/admin-login">Admin Login</Link> <SlLogin />
                      </Flex>
                    </MenuItem>
                  </MenuList>
                </Menu>
                ...
                <NavLink to="/login">
                  <SlUser size={30} color="#D7B14A" />
                </NavLink>
              )}
              {/* wishlist navlink */}
              <NavLink to="">
                <SlHeart size={30} color="#D7B14A" />
              </NavLink>
              {/* cart navlink */}
