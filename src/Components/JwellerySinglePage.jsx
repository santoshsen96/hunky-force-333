...
const JwellerySinglePage = () => {
  ...
  const GetData = async () => {
    let res = await axios.get(`http://localhost:8080/jewellery/${id}`);
    setData(res.data);
  };
    ...
    <div style={{ padding: "20px" }}>
      <Box w="80%" m="auto" mt="20px">
        <Breadcrumb separator={<RxDotFilled />}>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink href="/Jewelery">Jwelery</BreadcrumbLink>
          </BreadcrumbItem>
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            <AlertDialogBody m={"10px"} fontSize={"20px"}>
              {present ? "Item Already Present In Cart" : "Item Added To Cart"}
            </AlertDialogBody>
